...
macOS's TCC (Automation, app-data protection, Full Disk Access) can block reads even when writes work, and the "would like to access data from other apps" dialog can recur on every Claude Desktop restart. The verified fix is granting Full Disk Access to the exact interpreter Claude Desktop launches (`doctor`'s "Claude Desktop interpreter" check prints that path) - clicking Allow on the dialog does not fix the recurrence. That grant is broad - it applies to every program run with that interpreter, not just this server - so read [docs/MACOS_PERMISSIONS.md](docs/MACOS_PERMISSIONS.md)'s ["Risks of granting Full Disk Access to a Python interpreter"](docs/MACOS_PERMISSIONS.md#risks-of-granting-full-disk-access-to-a-python-interpreter) section before granting it, alongside the dialogs you'll see and the fix ladder for a headless Mac.
...
`doctor` prints exactly **one** "grant Full Disk Access" instruction, never two: "Interpreter identity" and "Claude Desktop interpreter" share a single resolution of the interpreter Claude Desktop will actually launch, computed once regardless of check order. When that resolution differs from the interpreter currently running doctor, "Interpreter identity" demotes to INFO with no grant instruction at all (so you never grant Full Disk Access to the wrong, doctor-only interpreter), and "Claude Desktop interpreter" carries the one unmistakable instruction - `GRANT FULL DISK ACCESS TO THIS FILE: <path>` as its first sentence - naming the file Claude Desktop actually launches. Text output ends with a `Full Disk Access target for Claude Desktop: <path>` footer line (one per resolved path); `--json` carries the same path(s) in a top-level `full_disk_access_targets` array. Clicking "Allow" on the "would like to access data from other apps" dialog does **not** persist for an interpreter launched by Claude Desktop (verified macOS 26.6) - only granting Full Disk Access to that exact file stops the dialog from recurring.

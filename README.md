...
macOS's TCC (Automation, app-data protection, Full Disk Access) can block reads even when writes work, and grants can be lost after a Claude Desktop restart or interpreter upgrade. `doctor`'s "Interpreter identity" and "Launch parent" checks pinpoint the exact cause - "Interpreter identity" WARNs for both `uv-managed` and Homebrew `framework` interpreters, since both embed a version-specific path that must be re-granted after every upgrade. See [docs/MACOS_PERMISSIONS.md](docs/MACOS_PERMISSIONS.md) for the dialogs you'll see, why they recur, and the fix ladder for a headless Mac.

...
- `success` - always `false` on a structured error.
- `error` - a short, stable, machine-readable UPPER_SNAKE_CASE code (e.g. `VALIDATION_ERROR`, `NOT_FOUND`, `TARGET_COMPLETED`, `NO_VALID_TAGS`, `INVALID_WHEN`, `INVALID_DEADLINE`, `NO_CHECKLIST_ITEMS`, `NO_TODO_IDS`, `TAG_CREATION_RESTRICTED`, `APPLESCRIPT_ERROR`). Safe to switch on; does not change wording across releases.
- `message` - a human-readable explanation, safe to display but not safe to pattern-match on (wording may change).
- Additional fields may be present depending on the error (e.g. `VALIDATION_ERROR` also carries `field` and `invalid_value`; an AppleScript-execution failure may carry `details` with the raw AppleScript error text; a URL-scheme auth-gate failure carries `hint`).

This shape is produced by a single shared implementation, `tools_helpers.errors.write_error(code, message, **extra)`, used at every layer that returns a write-tool structured error: `ThingsMCPServer._write_error` (server.py, the MCP tool boundary), `WriteOperations`/`BulkOperations` (`tools_helpers/write_operations.py`, `tools_helpers/bulk_operations.py`), and `parameter_validator.create_validation_error_response` (which independently produces the same `VALIDATION_ERROR` shape, predating this helper). Every AppleScript-execution-failure/exception path in `server.py`, `write_operations.py`, and `bulk_operations.py` now goes through `write_error`, carrying the dynamic AppleScript/exception text in a `details` field rather than overloading `error` with it. `move_operations.py` (`move_record`, `bulk_move_records`) already used UPPER_SNAKE_CASE codes (`VALIDATION_ERROR`, `TODO_NOT_FOUND`, `NO_TODOS_SPECIFIED`, `INVALID_DESTINATION`, `APPLESCRIPT_ERROR`, etc.) before this bead and needed no changes.

Known, currently-out-of-scope exceptions to this contract:
- **`scheduling/todo_operations.py`** (`TodoOperations`, backing `add_todo`/`update_todo`/`add_project`/`update_project`/checklist scheduling) still has ~26 human-string/`str(e)`-leak `error` sites not yet migrated - e.g. `_resolve_list_id`/`_resolve_list_title`'s `"list_id '...' does not match any known project or area"`/`"is ambiguous - matches multiple projects/areas: ..."`, `add_todo`'s `"heading requires a target project (list_id or list_title)"`, `update_todo`'s `"heading cannot be empty; ..."`, `add_project`/`update_project`'s `"when='evening' is not supported for projects; ..."`, `_add_todo_via_url_scheme`'s `"Todo could not be confirmed created within ...s"`, and multiple `result.get('error', 'Unknown error')`/`str(e)` passthroughs. This file is a much larger surface than `write_operations.py`/`bulk_operations.py` (many internal helper-return conventions reused across 6+ call sites) with extensive existing test coverage asserting the current human-text substrings - tracked as a follow-up bead (hq-f0w.46) rather than migrated here.
...
- The shared literal `"Things URL-scheme auth token not configured"` (sourced from `AppleScriptManager.execute_url_scheme`'s `AUTH_REQUIRING_ACTIONS` gate, `services/applescript_manager.py`) is used identically, verbatim, across `add_checklist_items`/`prepend_checklist_items`/`replace_checklist_items`/`update_todo`/`bulk_update_todos` and is exact-matched by tests in `test_when_evening_scheduling.py`/`test_url_scheme_auth_gate.py`/`test_update_todo_heading.py`. It is a stable, single-sourced literal in practice (not a `str(exception)` leak or a one-off human sentence), so it is left as-is rather than migrated to an UPPER_SNAKE code as part of this bead - doing so would be a cross-cutting change touching many call sites and tests at once, better suited to its own bounded task.

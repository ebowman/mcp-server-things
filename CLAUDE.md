...
**`database_access_denied`**: most list tools' internal `_get_*_sync` implementations (`tools_helpers/read_operations.py`) previously caught *any* exception from their underlying `things.py` call and either swallowed it (returning an empty `[]`/empty envelope) or let it propagate as a generic, unclassified exception (surfaced by `server.py`'s tool wrapper as a raised `ToolError` for most list tools, or `internal_error` for the handful of tools with their own structured-error fallback) - either way, a macOS TCC (Full Disk Access) denial reading the Things SQLite database was indistinguishable from any other failure. Each such `_get_*_sync` catch-all now checks `tools_helpers.read_operations.is_db_access_error(e)` first and, when it matches (`sqlite3.OperationalError` containing `'unable to open database file'`/`'authorization denied'`, or a plain `PermissionError`), re-raises immediately instead of swallowing it; `server.py`'s corresponding tool wrapper (e.g. `get_today`, `get_todos`) then catches that re-raised exception, classifies it again via the same helper, and returns the structured `database_access_denied` envelope (`read_error_from_exception`) instead of re-raising. Any other, unclassified exception is unaffected - it is still swallowed/returned as an empty result by the `_get_*_sync` layer, or re-raised as a `ToolError`/returned as `internal_error` by `server.py`, exactly as before this bead.

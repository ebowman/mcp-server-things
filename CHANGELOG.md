...
### Added
- **Opt-in live Things 3 smoke suite (`tests/live/`), and live-write gating for `tests/integration`'s real-Things fixtures** (hq-f0w.14) - nothing in the repo previously talked to a real, running Things 3 before release (CI runs `pytest tests/unit` only), and `tests/integration/conftest.py`'s `real_things_tools`/`cleanup_test_todos` fixtures constructed a real `AppleScriptManager` with no opt-in gate at all, so `pytest tests/integration` could silently write to (and delete from) whatever Things 3 database happened to be open. `tests/live/` is a new, self-cleaning suite: the whole directory is skipped unless `THINGS_MCP_LIVE_TESTS=1` is set AND Things 3 is actually running (a fast, non-launching AppleScript probe), a session-scoped fixture creates exactly one throwaway project (prefixed `hq-f0w-smoke live `) per run, and a finalizer trashes every item created during the session - even on failure - then verifies via `things.py` that each tracked id actually landed in the Trash, failing the teardown itself if anything is left behind. Covers: title/notes round-trip through both `get_todos`/`get_todo_by_id` and `things.py` directly (including commas/quotes/colons and multi-line notes), `update_project` multi-line notes, heading placement (skips with a clear reason since `add_project` has no `##Heading`-seeding support at all - its AppleScript path splits every `todos` payload line into a plain to-do, never a real heading), `get_anytime`/`get_someday`/`get_today`/`get_trash` never returning non-to-do types by default, `bulk_update_todos` with special-character titles and multi-line notes, `update_project(completed=/canceled=)` status transitions, checklist tools (clean auth-token-missing error, or a real round-trip when a token is configured), and completed-todo `completionDate`. `real_things_tools`/`cleanup_test_todos` are now gated behind the same `THINGS_MCP_LIVE_TESTS=1` env var, skipping with a clear reason before constructing a real `AppleScriptManager` - other, purely mock-based integration tests are unaffected. **Not yet gated:** `test_bulk_operations_comprehensive.py`, `test_search_comprehensive.py`, and `test_search_performance.py` build their own local fixtures instead of `real_things_tools` and can still write to a live Things 3 without the env var - tracked as hq-f0w.42. Teardown also sweeps every current child of the smoke project (`things.tasks(project=..., type=None, status=None, trashed=None)`) before deleting anything, since deleting a project does not cascade a trashed flag onto its own children - a to-do left under the project at delete time is otherwise orphaned (still `incomplete`, still pointing at the now-trashed project, never itself trashed); found live during review and fixed. New `live` pytest marker (registered in both `pytest.ini` and `pyproject.toml`), `make test-live` target, `docs/TESTING.md` (five-point testing policy plus the live-suite contract), and a mandatory live-suite-passes step added to the CLAUDE.md Release Process and Release Checklist before tagging. See `docs/TESTING.md` for the full policy.
- **Gated the last five live-writing integration files' own `AppleScriptManager`/`ThingsTools` fixtures behind `THINGS_MCP_LIVE_TESTS=1`** (hq-f0w.44, follow-up to hq-f0w.42) - `test_cleanup_mechanism.py`, `test_date_scheduling_integration.py`, `test_month_edge_cases.py`, and `test_temporal_queries.py` each built a real `AppleScriptManager` in a local fixture or inline per test, relying only on also requesting the already-gated `cleanup_test_todos` fixture; each is now marked `live` (skipped at collection without the env var) and each local fixture/test additionally calls `_require_live_tests_env()` directly, so dropping the `cleanup_test_todos` dependency can no longer un-gate a real write. `verify_cleanup.py` (a standalone `__main__` script, not collected by pytest) now exits with an error unless `THINGS_MCP_LIVE_TESTS=1` is set. `pytest tests/integration tests/live` without the env var: 130 skipped in ~0.2s with zero `osascript` calls (verified with a scratch subprocess-raising plugin).
...
- **Opt-in live Things 3 smoke suite (`tests/live/`), and live-write gating for `tests/integration`'s real-Things fixtures** (hq-f0w.14) - nothing in the repo previously talked to a real, running Things 3 before release (CI runs `pytest tests/unit` only), and `tests/integration/conftest.py`'s `real_things_tools`/`cleanup_test_todos` fixtures constructed a real `AppleScriptManager` with no opt-in gate at all, so `pytest tests/integration` could silently write to (and delete from) whatever Things 3 database happened to be open. `tests/live/` is a new, self-cleaning suite: the whole directory is skipped unless `THINGS_MCP_LIVE_TESTS=1` is set AND Things 3 is actually running (a fast, non-launching AppleScript probe), a session-scoped fixture creates exactly one throwaway project (prefixed `hq-f0w-smoke live `) per run, and a finalizer trashes every item created during the session - even on failure - then verifies via `things.py` that each tracked id actually landed in the Trash, failing the teardown itself if anything is left behind. Covers: title/notes round-trip through both `get_todos`/`get_todo_by_id` and `things.py` directly (including commas/quotes/colons and multi-line notes), `update_project` multi-line notes, heading placement via `add_project`'s `##Heading`-seeding support (hq-f0w.41), `get_anytime`/`get_someday`/`get_today`/`get_trash` never returning non-to-do types by default, `bulk_update_todos` with special-character titles and multi-line notes, `update_project(completed=/canceled=)` status transitions, checklist tools (clean auth-token-missing error, or a real round-trip when a token is configured), and completed-todo `completionDate`. `real_things_tools`/`cleanup_test_todos` are now gated behind the same `THINGS_MCP_LIVE_TESTS=1` env var, skipping with a clear reason before constructing a real `AppleScriptManager` - other, purely mock-based integration tests are unaffected. **Not yet gated:** `test_bulk_operations_comprehensive.py`, `test_search_comprehensive.py`, and `test_search_performance.py` build their own local fixtures instead of `real_things_tools` and can still write to a live Things 3 without the env var - tracked as hq-f0w.42. Teardown also sweeps every current child of the smoke project (`things.tasks(project=..., type=None, status=None, trashed=None)`) before deleting anything, since deleting a project does not cascade a trashed flag onto its own children - a to-do left under the project at delete time is otherwise orphaned (still `incomplete`, still pointing at the now-trashed project, never itself trashed); found live during review and fixed. New `live` pytest marker (registered in both `pytest.ini` and `pyproject.toml`), `make test-live` target, `docs/TESTING.md` (five-point testing policy plus the live-suite contract), and a mandatory live-suite-passes step added to the CLAUDE.md Release Process and Release Checklist before tagging. See `docs/TESTING.md` for the full policy.
- **`get_project_headings(project_id, mode?)` - new read-only tool for a project's heading structure** (GH#9 item 3c) - things.py supports `things.tasks(type='heading', project=...)` but no tool exposed it; the nearest workaround, `search_advanced(type='heading')`, returns headings without their to-do counts. Returns `{uuid, title, index, todoCount}` per heading, in Things' own display order, where `todoCount` is the number of open to-dos directly under that heading (`things.todos(heading=uuid)`). An unknown `project_id`, or one that resolves to something other than a project (area/to-do/heading), returns a structured error (`{"success": false, "error": "not_found"|"invalid_type", "message": ...}`) instead of raising. Read-only by design: headings can only be created at project-creation time (`add_project`'s `##` lines, see hq-f0w.41) or by placing a to-do under an existing heading (`add_todo(..., heading=...)`) - there is no public Things 3 API to rename or delete a heading (no AppleScript heading class).
...
### Removed
- **Remaining zero-caller AppleScript formatter/manager helpers and inert conftest mock responses removed** (hq-f0w.38, from hq-f0w.33 review) - `AppleScriptFormatters.split_applescript_output`, `.has_reminder_time`, and `.extract_reminder_time` (`services/applescript/formatters.py`) had no callers anywhere in `src/` or `tests/` (`has_reminder_time` was previously only called by the now-also-dead `extract_reminder_time`). `AppleScriptManager`'s thin delegate wrappers `_parse_applescript_date`, `get_applescript_date_formatter`, `format_applescript_date_to_iso`, `_parse_applescript_tags`, and `_build_things_url` (`services/applescript_manager.py`) were likewise uncalled - `execute_url_scheme` builds URLs via `self.formatters.build_things_url` directly, not the `_build_things_url` wrapper, so `build_things_url` itself is kept. Also removed three inert `mock_applescript_manager.set_mock_response(...)` calls for `"get_todos"`/`"get_projects"`/`"get_areas"` in `tests/conftest.py`'s `mock_applescript_manager_with_data` fixture - `MockAppleScriptManager` has had no `get_todos`/`get_projects`/`get_areas` methods since hq-f0w.33, so nothing ever read those `mock_responses` keys back; the fixture's other mock responses (`url_add`/`url_update`) and its `sample_todo_data`/`sample_project_data`/`sample_area_data` fixture dependencies are unchanged. No behavior change - all removed code was unreachable. `docs/PHASE_2_COMPLETION_REPORT.md`, `docs/REFACTORING_PLAN.md`'s Phase 2 section, and `CLAUDE.md`'s "Active Refactoring Plan" parser bullets are now marked superseded/obsolete - the state-machine `parser.py` and `use_new_applescript_parser` flag they describe don't exist in the current codebase (removed in hq-f0w.33).
- **Dead AppleScript read-parser stack removed** (hq-f0w.33) - hq-nxu.8 removed the AppleScript read path (`get_todos`/`get_projects`/`get_areas` AppleScript methods and `services/applescript/queries.py`), leaving these with zero production callers: `services/applescript/parser.py` (`AppleScriptParser`, the state-machine parser) and its export from `services/applescript/__init__.py`; `AppleScriptManager._parse_applescript_list` and the ~230-line legacy string-manipulation parser body it dispatched to, plus the wrapper methods only that body used (`_split_applescript_output`, `_has_reminder_time`, `_extract_reminder_time`, `_enhance_record_with_reminder_info`); the `use_new_applescript_parser` config flag (`config.py`) and its boot-time log lines choosing between the new/legacy parser; `AppleScriptFormatters.enhance_record_with_reminder_info`; and `ToolsHelpers.convert_applescript_todo` (`tools_helpers/helpers.py`). `tests/unit/test_applescript_parser.py` and `test_parser_comparison.py`, which existed solely to exercise the deleted parser, are removed, along with matching dead stub methods (`get_todos`/`get_projects`/`get_areas`/`get_todos_due_in_days`/`get_todos_activating_in_days`/`get_todos_upcoming_in_days`) on `tests/conftest.py`'s `MockAppleScriptManager` that mocked an interface the real `AppleScriptManager` no longer has. No behavior change: `AppleScriptManager` no longer parses AppleScript list output at all (nothing called it), and reads continue to go through things.py exclusively, as they have since hq-nxu.8.

### Fixed
- **Small hq-nxu.9 review follow-ups: month-overflow regex/comments, `_parse_tag_list_for_update` test coverage, CLAUDE.md fixes** (hq-f0w.25) - reworded two `scheduling/todo_operations.py` comments (`update_todo`/`update_project`'s deadline-clear path) that had accidentally embedded the literal string `set due date of X to missing value`, which had forced `test_month_overflow_workaround.py`'s completeness check to a narrower regex than intended; restored the original broader `set due date of` pattern now that the comments no longer collide with it. Added direct unit tests for `server._parse_tag_list_for_update` (`''` -> `[]`, `'a,,b'` -> `['a', 'b']`, `' , '` -> `[]`), mirroring the existing `_parse_tag_list` coverage. Fixed a mislabeled `# WRONG` comment in CLAUDE.md's clear-fields section that actually showed correct `when='anytime'` usage, and documented that whitespace-only `notes`/`tags` values (e.g. `notes="   "`, `tags=" , "`) are also treated as explicit clear requests, same as `''`.
- **The last two allowlisted success-only tests in `test_meta_assertion_quality.py` are retrofitted; `ALLOWLIST` is now empty** (hq-f0w.31) - `test_tag_management_comprehensive.py::test_empty_tag_string` now asserts the `NO_VALID_TAGS` error code and that `execute_applescript` was never called (rather than only `success is False`); `test_url_scheme_auth_gate.py::test_with_token_succeeds` (all three sibling classes: `TestAddChecklistItemsAuthGate`, `TestPrependChecklistItemsAuthGate`, `TestReplaceChecklistItemsAuthGate`) now asserts the emitted URL contains `auth-token=` and the tool-specific checklist param (`append-checklist-items`/`prepend-checklist-items`/`checklist-items`). Tests only; no production code changed.
- **Checklist tools now surface the auth-token `hint` on failure, and the dead ungated `things:///update` fallback in `reliable_scheduling.py` is removed** (hq-f0w.20, from hq-f0w.12 review) - `add_checklist_items`/`prepend_checklist_items`/`replace_checklist_items` forwarded `execute_url_scheme`'s `error` on failure but dropped its `hint` (the actionable "add a `.things-auth` file..." guidance returned when no auth token is configured), so callers hitting the auth gate got an error with no remediation path; all three now forward `hint` when present, matching `update_todo`'s heading/evening URL-scheme path (already correct since hq-f0w.7/nxu.10). Separately, `reliable_scheduling.py`'s `ReliableThingsScheduler` - unused by any production code path (the live scheduler is `scheduling/strategies.py`) - had a "Layer 1" `things:///update` URL built and executed via a bare `subprocess.run(['open', url])`, bypassing the auth-token gate and hint machinery entirely; that dead `_try_url_scheme_scheduling`/`_execute_url_scheme` path (and its now-unused `subprocess`/`urllib.parse.quote` imports) is removed, leaving the AppleScript-date-objects and list-assignment fallback layers intact. `.env.example`'s auth-token section previously said the token was used for "all URL scheme operations (checklist management, todo creation with checklists, etc.)"; corrected to say it's required only for `things:///update`-based operations (checklist tools, `update_todo`'s heading/evening paths) - `add_todo` with `checklist_items` uses the unauthenticated `things:///add` action and is unaffected.
- **Whitespace-only `when` (e.g. `when='   '`) is now rejected with the same structured hint as `when=''`, instead of silently becoming a no-op** (hq-f0w.34, found by the hq-nxu.10 reviewer) - the 5 `when`-accepting tools (`add_todo`, `update_todo`, `bulk_update_todos`, `add_project`, `update_project`) pre-validated `when` in server.py with `if when:` (true for `'   '`) and assigned the return of `ParameterValidator.validate_date_format(when, ..., allow_relative=True)`, which strips the string and returns `None` for whitespace-only input without raising - silently treating `when='   '` as "not provided" instead of the hq-nxu.9 structured rejection (`{"success": False, "error": "VALIDATION_ERROR", "field": "when", "message": "use when='anytime' or when='someday' to unschedule"}`) that `when=''` already produced. All 5 sites now reject whitespace-only `when` explicitly, before normalization, with that same structured error. Before: `update_todo(id=..., when='   ')` → `{"success": true}` with the todo's schedule unchanged. After: `update_todo(id=..., when='   ')` → structured `VALIDATION_ERROR` naming `when`. Also fixed: `ParameterValidator.validate_date_format`'s rejection message claimed `"...or a relative date (today, tomorrow, etc.)"` even when called with `allow_relative=False` (as `deadline` validation always is), where relative dates are in fact rejected - the message now reads `"must be in YYYY-MM-DD format, got '...'"` when `allow_relative=False`.
- **`get_today`/`get_inbox`/`get_upcoming`/`get_anytime`/`get_someday` now report a concrete `mode` when `mode` is omitted, instead of the literal `None`** (hq-f0w.39, found by the hq-f0w.26 reviewer) - each of these five tools gated its response-mode optimization behind `if mode:`, so an omitted `mode` skipped `context_manager.optimize_response` entirely and `structured_content['mode']` was left as `None` - inconsistent with CLAUDE.md's documented contract that an omitted (or `'auto'`) mode reports the concrete mode AUTO resolved to. All five call sites (including the separate `days=` branch of `get_upcoming`) now always route through `context_manager.optimize_request`/`optimize_response`, treating an omitted `mode` the same as `mode='auto'`; `requested_mode` still correctly preserves `None` for an omitted request versus `'auto'` for an explicit one. Before: `get_today()` → `{"mode": None, "requested_mode": null}` (mode simply absent from optimization). After: `{"mode": "standard", "requested_mode": null}` (or whatever concrete mode AUTO/the tool's default resolves to). `hq-nxu.3`'s pre-limit `total` pagination contract and `hq-f0w.26`'s empty-result AUTO resolution are unaffected by this change.
- **`get_areas(mode='minimal')` no longer drops an area's `tags`; `get_areas`/`get_projects(include_items=true)` no longer drop the nested `projects`/`todos` keys under `minimal`/`standard`; mixed-list project rows (`include_projects=true`, `search_advanced`) now carry `area`/`areaTitle`** (hq-f0w.37, found by the hq-f0w.32 implementer/reviewer) - three related field-filtering gaps in `context_manager.py`/`read_operations.py`. (1) `get_areas` had no area-shaped field set, so it fell through to the todo-shaped `TODO_FIELD_SETS`, whose `minimal` entry has no `tags` key - `get_areas(mode='minimal')` silently returned areas with no `tags`. A new `AREA_FIELD_SETS` (identical across `summary`/`minimal`/`standard` - `uuid`/`title`/`type`/`tags` - since `convert_area`'s output is a fixed 4-key schema) is now used when `method_name == 'get_areas'`. (2) `get_areas(include_items=true)`/`get_projects(include_items=true)` attach nested `projects`/`todos` lists directly in `read_operations.py`, independent of mode, but `minimal`/`standard`'s field sets never listed those keys, so they were silently dropped at those modes (only `detailed`/`raw`, which skip filtering entirely, kept them) - `_apply_field_filtering` now preserves any `projects`/`todos` key already present on a row for `get_areas`/`get_projects`, in every mode. (3) `get_today`/`get_upcoming`/`get_anytime`/`get_someday(include_projects=true)` and `search_advanced` converted every row - including project rows - via `ToolsHelpers.convert_todo`, whose output schema never includes `area`/`areaTitle`; a new `convert_item()` helper in `read_operations.py` dispatches each raw row to `convert_project`/`convert_todo` based on its own `type` before conversion, and `_apply_field_filtering` now also filters any row with `type == 'project'` against `PROJECT_FIELD_SETS` regardless of the calling method, so project rows in these mixed lists keep `area`/`areaTitle` at every mode. Before: `get_areas(mode='minimal')` → no `tags`; `get_projects(include_items=true, mode='standard')` → no `todos` key; `get_today(include_projects=true)` on a project due today → no `area`/`areaTitle`. After: all three present. `get_projects(include_items=true)` remains documented as context-expensive on large databases regardless of mode - this fix does not change that guidance (see CLAUDE.md "Known Limitations").
- **`get_trash(include_projects=true)` project rows now carry `area`/`areaTitle`; `get_optimization_capabilities()` now also advertises `area_field_sets`** (hq-f0w.45, found by the hq-f0w.37 implementer) - `_get_trash_sync` was the one mixed-list call site hq-f0w.37 missed: it still converted every row (to-do or project) via `ToolsHelpers.convert_todo`, whose output never includes `area`/`areaTitle`, instead of the `convert_item()` dispatcher already used by `get_today`/`get_upcoming`/`get_anytime`/`get_someday`/`search_advanced`. `_get_trash_sync` now uses `convert_item()` too, so a trashed project row carries `area`/`areaTitle` like the other mixed lists; the pre-limit `total_count` and heading-backfill pass are unchanged. Separately, `get_optimization_capabilities()`'s `dynamic_field_filtering` advertised `field_sets` (todo) and `project_field_sets` but never `area_field_sets` (added for `get_areas` in hq-f0w.37 but not surfaced here) - it now also includes `area_field_sets`, derived from the same `AREA_FIELD_SETS` used for actual filtering. Before: `get_trash(include_projects=true)` on a trashed project → no `area`/`areaTitle`; `get_optimization_capabilities()` → no `area_field_sets` key. After: both present.
- **`delete_todo` can now delete a project id (previously errored)** (hq-f0w.40, found by the hq-f0w.14 implementer while writing `tests/live` teardown) - `delete_todo` always ran `delete (to do id "...")`, but Things' AppleScript dictionary does not treat a project as a to-do subtype for `delete`: that script reliably errors with `"Can't get to do id ..."` for a project id even though reads on that same id resolve fine (only `delete (project id "...")` works). `delete_todo` now resolves the id's type via `things.get(id, trashed=None)` first and picks the matching script (`to do id` vs `project id`); if things.py can't resolve the id (import failure, or the lookup itself can't be completed) it falls back to trying `to do id` then `project id` blind, and if both `delete` forms fail it falls back further to `move ... to list "Trash"` (handles a to-do whose parent project was already trashed in the same pass, where `delete` itself errors but `move` still resolves the id). Headings, areas, and tags cannot be deleted via AppleScript's `delete` command at all; those ids now return a structured `not_deletable` error (naming the correct manual UI workflow) instead of attempting - and failing - an AppleScript call. A genuinely nonexistent id now returns a structured `not_found` error immediately instead of spending 2-3 AppleScript round-trips on an id that will never resolve. `delete_todo` still trashes rather than permanently deleting (its docstring previously didn't say so); it now also documents that it accepts a project id. Before: `delete_todo(project_id)` → AppleScript error surfaced as a generic failure. After: `delete_todo(project_id)` → project trashed successfully. Follow-up fix (same bead, review round 2): things.py 1.0.1's `things.get(uuid, **kwargs)` forwards `**kwargs` to `areas()`/`tags()` after `tasks()` rules an id out via `ValueError`, but `Database.get_areas()` rejects the `trashed` kwarg that `tasks()` accepts - so `things.get(id, trashed=None)` raised `TypeError` for *every* non-task id (areas, tags, nonexistent ids), not just when things.py was genuinely unavailable; this was being swallowed as "unresolved" and routed into the blind-fallback path, so real area/tag ids never reached `not_deletable` (3 wasted AppleScript round-trips instead) and every bogus id did too. Fixed: a `TypeError` from the `trashed=None` call now retries once with a bare `things.get(id)` (kwargs-free, since `tasks()` has already ruled itself out) before falling back to "unavailable" for any further error.
- **Empty results with `mode` omitted or `'auto'` now report a concrete effective mode instead of the literal string `"auto"`** (hq-f0w.26, found by the hq-f0w.6 reviewer) - `context_manager.optimize_response`'s empty-data path (`if not data:`) returned before AUTO mode selection ever ran, so `structured_content['mode']` on an empty result (e.g. `get_projects` on an empty list, `get_project_headings` on a project with no headings) was the literal `"auto"`, contradicting CLAUDE.md's Structured Output docs ("never the literal string `auto`"). The empty path now resolves AUTO to `"standard"` (there's no data to size-select against) and records it in `meta['mode']`, the same place `ThingsMCPServer._read_result` already looks for the effective mode. Before: `get_projects()` on an empty database → `{"items": [], "mode": "auto", "requested_mode": "auto"}`. After: `{"items": [], "mode": "standard", "requested_mode": "auto"}`.
- **`search_advanced` no longer mislabels an unrelated `ValueError` as `unknown_tag` just because a `tag` filter was also supplied** (hq-f0w.18, found by the hq-nxu.5 reviewer) - `_search_advanced_sync`'s `except ValueError` branch treated *any* `ValueError` from `things.py` as an unknown-tag error whenever a `tag` filter was present, regardless of the error's actual cause. Live repro: `search_advanced(tag='llm-wiki', start_date='2024-1-5')` - `start_date` passes the server's own `strptime` pre-check but `things.py`'s internal date validator rejects the non-zero-padded month/day, raising `ValueError("Invalid start_date argument: '2024-1-5'")`; this was reported back as `{"error": "unknown_tag", "tag": "llm-wiki", "suggestions": [...]}` even though `llm-wiki` is a valid, existing tag. The branch now only treats the error as `unknown_tag` when `tag` is set **and** the message contains things.py's actual `validate()` text, `"Unrecognized tag type"`; any other `ValueError` (bad `start_date`/`deadline` value or operator, etc.) re-raises into the outer handler, which now returns `{"success": false, "error": "invalid_parameter", "message": str(e)}` instead of silently swallowing it as an empty result (other, non-`ValueError` exceptions are unaffected and still return an empty list, matching prior behavior). Also fixed in passing: a stale `server.py` comment near the `search_advanced` structured-error short-circuit referenced a since-removed error shape, and a redundant `tagged_items['tag'] = tag` in `get_tagged_items` (the value is already set by `_build_unknown_tag_error`) was removed.
- **Retired the last ad-hoc AppleScript string escaper on the surviving code path** (hq-f0w.21, from hq-f0w.2 review) - `ValidationService._escape_id()` (used to build `to do id "..."`/`project id "..."`/`area id "..."` lookups) still built its own `.replace('\\', '\\\\').replace('"', '\\"')` escaping inline instead of delegating to the shared escaper. It now delegates to `AppleScriptTemplates.escape_string_inner()` (the shared escaper `add_todo`/`update_todo`/`add_project`/`update_project` already used since hq-f0w.2), so `_escape_id`'s output is identical to every other AppleScript-string-building call site. No behavior change for the ID values this method actually receives in practice (alphanumeric UUIDs); this closes the gap for the general case. (`AppleScriptManager.update_project_direct()` carried a similar ad-hoc escaper but had zero production callers; it is being removed in hq-f0w.22 along with the method itself, rather than patched here.)
- **`update_todo`/`bulk_update_todos`/`update_project` now share one `completed`/`canceled` status-precedence table, and completed/canceled parsing at the MCP boundary is now strict** (hq-f0w.22, from hq-f0w.11 review) - `update_todo`'s AppleScript builder (`_build_update_script`) treated `canceled=False` alone (with `completed` omitted) as a silent no-op, while `update_project` (hq-f0w.11) already reopened the project in that same case; `bulk_update_todos`'s builder gated on canceled-is-not-None first and emitted `open` for completed=true + canceled=false; it is now reordered to match. `_build_update_script` now matches `update_project`/`bulk_update_todos`: `canceled=True` always wins (regardless of `completed`); otherwise `completed=True`/`completed=False` sets completed/open; otherwise `canceled=False` alone reopens; both omitted leaves status unchanged. Separately, `server.py`'s `completed`/`canceled` string-to-bool conversion used `value.lower() == 'true'`, which silently turned any non-`'true'` string - including typos like `'yes'`/`'1'` - into `False` and could unintentionally reopen a completed/canceled item. All three tools' MCP boundary now uses a new strict parser (`_parse_strict_bool`) that only accepts an actual bool or the strings `'true'`/`'false'` (case-insensitive, whitespace-trimmed); anything else returns a structured `{"success": false, "error": "VALIDATION_ERROR", "field": "completed"|"canceled", "message": ...}` error instead of guessing. Also removed `AppleScriptManager.update_project_direct` (dead code, no callers - had its own divergent completed-wins precedence). Before: `update_todo(id, canceled='false')` → `success: true`, status unchanged (silent no-op); `update_todo(id, completed='yes')` → `success: true`, todo reopened. After: `update_todo(id, canceled='false')` → todo reopened (`things.get(uuid)['status'] == 'incomplete'`); `update_todo(id, completed='yes')` → `{"success": false, "error": "VALIDATION_ERROR", "field": "completed", "message": "must be 'true' or 'false' (case-insensitive), got 'yes'"}`.
- **Heading existence checks no longer miss completed headings; `add_todo`/`update_todo` now reject a write into a completed/canceled heading or project instead of silently reopening it** (hq-f0w.36) - `things.tasks(type='heading', project=...)` defaults to `status='incomplete'`, so a completed heading (e.g. a past-dated recurring heading like "Johan" in a project) was invisible to `_check_heading_exists`, producing a false "heading ... was not found" warning even though the URL-scheme move actually succeeded (same root cause/fix as the heading map built in hq-f0w.24); it now passes `status=None`. Separately, `add_todo`/`update_todo` now pre-check the *status* of the resolved target - not just its existence - before any write: if the matched heading, or the resolved target project (`list_id`/`list_title`), is `completed`/`canceled`, the call is rejected with a structured error (`{"success": false, "error": "TARGET_COMPLETED", "message": "..."}`) before any AppleScript or URL-scheme write is issued, instead of silently reopening the project/heading in Things (a real, visible change to pre-existing user data). This check also covers `add_todo(list_id=..., checklist_items=[...])` and `add_todo(list_id=..., when='evening')` with **no** `heading` - both still take the Things-URL-scheme create path (for the checklist/evening flag, unrelated to the project target), so a completed/canceled `list_id` project is pre-checked there too, not only on the heading branch. There is no `allow_reopen` override yet; reopen the target manually in Things first, or choose another target. Areas have no completed/canceled concept in Things and are unaffected. The heading-exists warning and the new heading-completion check share a single `things.tasks(status=None)` query per call (folded into `_check_heading_status`) rather than querying twice. Also note for API callers: things.py's SQLite reads can lag a URL-scheme write by roughly 1-2s (the write itself is async), so a read-back immediately after a URL-scheme create/update may still reflect pre-write state - see the note in CLAUDE.md.
- **Removed the tests-only-referenced `url`/`status` kwargs from `add_todo`'s test suite** (hq-f0w.30) - `url` and `status` were never real parameters anywhere in the tools layer (`add_todo(title, **kwargs)` throughout `tools.py`/`write_operations.py`/`scheduling/todo_operations.py`) and were never exposed by the `add_todo` MCP tool in `server.py`, so passing them was always a silent no-op reachable only via direct `ThingsTools` calls, not through the MCP tool itself; Things 3 to-dos have no URL field (put a link in `notes` instead) and no tentative/confirmed status concept. The 4 stale `xfail(strict=True)` tests asserting this dead behavior (`test_create_todo_with_url`, `test_url_with_special_chars`, `test_create_with_status_tentative`, `test_create_with_status_confirmed`) are removed.
- **`update_todo` can now move a to-do to a project or area (`list_id`/`list_title`), and the by-name AppleScript branches it already had are fixed to move by id** (hq-nxu.13) - `_build_update_script` had unreachable `area`/`project` AppleScript branches (no caller ever populated them) that also emitted `project <name>`/`area <name>` by *name*, inconsistent with `add_todo`'s id-based `project id "..."`/`area id "..."`. `update_todo` now accepts `list_id` (a project or area UUID, resolved the same way as `add_todo` via `_resolve_list_id`) and a new `list_title` (exact-title match via `_resolve_list_title`, used only when `list_id` is absent - an unknown or ambiguous title is a structured error, same as `add_todo`). Without `heading`, this moves the to-do directly into that project/area via AppleScript, applied in the same write as other AppleScript-only fields; the by-name branches now emit `project id "..."`/`area id "..."` for any resolved id, matching `add_todo`. With `heading` also given, `list_id`/`list_title` are instead resolved and consumed by the existing `things:///update` URL-scheme move-and-place-under-heading path added in hq-f0w.7 (this move is limited to a project or area destination - `update_todo` still cannot target the inbox/today/anytime/someday lists; use `move_record()` for those, as documented in the tool's docstring). Three gaps found in review before this landed, all fixed: (1) `when='evening'` combined with `list_id` and no `heading` previously double-applied the move - once via the AppleScript write, once more via `list-id` on the following evening URL-scheme call; `list_id`/`list_title` is now only sent on the URL-scheme call when `heading` is also present. (2) `heading` + `list_title` (no `list_id`) previously silently dropped `list_title` entirely (only `list_id` was wired into the heading path); `list_title` is now resolved via `_resolve_list_title` on the heading path too, with the same unknown/ambiguous structured errors, and used as `list-id` on the `things:///update` call. (3) an unknown `list_id` on the heading path previously wasn't pre-checked and was sent to Things as `list-id=<bogus>` (a pre-existing hq-f0w.7 silent no-op, no error surfaced); `list_id` (and now `list_title`) on the heading path is now pre-checked via `_resolve_list_id`/`_resolve_list_title` before any write, returning the same structured error as the non-heading move path. A fourth gap found in a follow-up review: that pre-check itself was implemented inside the `things:///update` URL-scheme block, which runs *after* the AppleScript write - so `update_todo(id=..., heading=..., list_id="bogus", title="New")` still applied `title` via AppleScript before returning the structured error. The heading-path `list_id`/`list_title` pre-check now runs in the same pre-write block as the non-heading move resolution, before `skip_applescript` is even computed, so the structured error is returned with no write of any kind, matching every other error path in `update_todo`.
- **`get_projects(mode='standard')` and `mode='minimal'` no longer drop a project's `area`/`areaTitle`** (hq-f0w.32, found by the hq-f0w.29 reviewer) - `context_manager._apply_field_filtering` used one shared `TODO_FIELD_SETS` for both to-do and project rows; that set was built for to-do rows, which never carry an `area` key, so it had no `area`/`areaTitle` entries and every project response under `minimal`/`standard` mode silently lost its area - only `detailed`/`raw` showed it. Project rows are now filtered against a separate `PROJECT_FIELD_SETS` (used when the response's `method_name` is `get_projects`), which includes `area`/`areaTitle`/`start`/`startDate`/`reminderTime` in `standard` mode and `area`/`start` in `minimal` mode; `get_optimization_capabilities()` now also advertises these under a new `project_field_sets` key alongside the existing todo `field_sets`, both still derived from their single source-of-truth class attribute so the two can't drift. `get_areas()`'s own top-level area rows (`uuid`/`title`/`type`/`tags`) are unaffected by this fix and continue to use `TODO_FIELD_SETS`. Before: `get_projects(mode='standard')` on a project filed under "Work" → no `area`/`areaTitle` key. After: `area`/`areaTitle` present.
- **`get_logbook` now includes canceled to-dos alongside completed ones (matching the Things app's own Logbook view), and its `limit` cap is raised from 100 to 500** (read sweep C7, hq-nxu.6) - `_get_logbook_sync` only fetched `things.todos(status='completed')`, so canceled to-dos never appeared in `get_logbook` even though the Things app's Logbook list shows both (live: 28 canceled to-dos in the database, all previously invisible to this tool). It now fetches both `status='completed'` and `status='canceled'` and merges them by `stop_date` (most recent first, same as before); each item's existing `status` field (`'completed'`/`'canceled'`) lets callers tell them apart. A new `include_canceled: bool = True` parameter (default preserves the new merged behavior) restores the completed-only view when set to `false`. `limit`'s cap is raised from 100 to 500 (matching `search_todos`/`search_advanced`) now that `offset` (hq-nxu.3) makes results past the first page reachable without it. Before: `get_logbook(period='365d')` on a database with 28 canceled to-dos in-period → 0 of them present. After: all 28 present, each with `"status": "canceled"`.
...
- **`get_todo_by_id` now also resolves area uuids, and returns a structured error for tag uuids instead of a misleading to-do-shaped dict** (hq-f0w.23, from hq-f0w.4 review) - `things.get(uuid)` (things.py 1.0.1) falls through `tasks()` → `areas()` → `tags()` in turn, so it also resolves area and tag uuids, not just to-do/heading/project ones; previously both fell through to `convert_todo` and came back as a to-do-shaped dict with `type: 'area'`/`type: 'tag'` and every to-do field empty. An area uuid now dispatches to `convert_area` and returns a proper area dict (`{uuid, title, type: 'area', tags}`). A tag uuid now returns a structured error (`{"success": false, "error": "invalid_type", "message": "... is a tag, not a retrievable item. Use get_tags() or get_tagged_items() for tags."}`) instead of an item, since a tag is a label with no content of its own - `things.get()` returns only `{uuid, type, title, shortcut}` for a tag, nothing worth surfacing as a "todo". Also dropped the redundant `things.checklist_items(todo_id)` re-fetch for to-dos: `things.get(uuid)` internally forces `include_items=True` for any single-uuid lookup (things.py 1.0.1), so `item['checklist']` is already the full list of checklist-item dicts by the time `_get_todo_by_id_sync` runs; it's normalized to the same `{'title', 'status'}` shape as before, just without the extra database round trip. `get_todo_by_id` has always guaranteed a to-do result carries a `checklist` list (`[]` when the to-do has no checklist) - things.py omits the `checklist` key entirely (rather than an empty list) for a checklist-less to-do, and that case is still normalized to `checklist: []` here, unchanged from before this bead. Before: `get_todo_by_id(<area uuid>)` → `{"type": "area", "title": null, "uuid": "...", "status": null, ...to-do fields...}`. After: `{"uuid": "...", "title": "King", "type": "area", "tags": []}`. Before: `get_todo_by_id(<tag uuid>)` → a similarly empty to-do-shaped dict with `type: "tag"`, nested under `item` at the MCP tool boundary (`{"item": {"type": "tag", ...}}`). After: `{"success": false, "error": "invalid_type", "message": "..."}` at the top level of the tool response - `get_todo_by_id`'s MCP tool wrapper detects the canonical structured-error shape (`success: false`) and returns it as-is instead of nesting it under `item`.
...
- **`add_project`'s `todos` payload now supports real `##Heading` lines, and the AppleScript path now verifies every requested to-do was actually created** (hq-f0w.41, found by hq-f0w.14) - `add_project`'s AppleScript path (`_build_create_project_script`) has no heading concept at all: a `todos` line like `"##Phase 1"` created a plain to-do literally titled `"##Phase 1"`, not a heading, and the codebase's own docs (CLAUDE.md, README, `get_project_headings`'s docstring) incorrectly claimed the documented `add-project` URL-scheme `to-dos` param turned such lines into headings - live-verified for this bead that it does not; `to-dos` creates only plain to-dos, same as the AppleScript path. The only Things API that can create a real heading at project-creation time is the `things:///json` URL-scheme action's structured `items` array (`{"type": "heading", ...}` / `{"type": "to-do", ...}`), live-verified to correctly nest subsequent to-dos under the most recently seen heading, and to accept the same `notes`/`tags`/`deadline`/`area`/`area-id` fields `add_project` already supports. `add_project` now routes the whole call through a new `_add_project_via_url_scheme` (mirroring `_add_todo_via_url_scheme`'s before/after id-snapshot-and-poll pattern, since `things:///json` doesn't return the created project's id) whenever the `todos` payload contains at least one `##` line; a `todos` payload with no `##` lines still uses the faster AppleScript path, unchanged. An empty heading title (a bare `"##"` line) is rejected with a structured error rather than silently creating an untitled heading. Separately, the bead's reported "second todo line silently dropped" symptom (10 AppleScript-path creates with 3-5 todo lines each, live, both throttled and back-to-back) could not be reproduced - most likely a one-off read-after-write race in a caller's own verification (`things.py` reads its on-disk SQLite snapshot, which can lag a fresh AppleScript write by 1-2s, same class of issue already documented for URL-scheme writes) rather than an actual AppleScript drop - but `_build_create_project_script`'s AppleScript now returns `(count of to dos of newProject)` alongside the project id from the same call that created it (no separate, potentially-stale read), so `add_project`'s response includes `todos_created` whenever `todos` was non-empty, with a `warnings` entry if fewer to-dos were created than requested. The URL-scheme path verifies `todos_created`/`headings_created` the same way, via a fresh `things.py` read (`things.todos(project=id)`, `things.tasks(type='heading', project=id, status=None)`) once the id-lookup poll has confirmed the created project's id, falling back to the requested counts if that verification read itself fails. Before: `add_project(todos="##Heading\nTask")` → one to-do literally titled `"##Heading"` plus a `"Task"` to-do, both un-nested, `{"success": true, "project_id": "..."}` with no way to tell if anything was dropped. After: a real "Heading" heading with "Task" nested under it, and `{"success": true, "project_id": "...", "todos_created": 1, "headings_created": 1}`.
...
- **`tests/integration/test_bulk_operations_comprehensive.py`, `test_search_comprehensive.py`, and `test_search_performance.py` can no longer write to a live Things 3 database without opting in** (hq-f0w.42, found by hq-f0w.14) - these three files built their own local `applescript_manager`/`things_tools` fixtures instead of the gated `real_things_tools`/`cleanup_test_todos` fixtures in `tests/integration/conftest.py`, so `pytest tests/integration` could perform real AppleScript writes/deletes with no `THINGS_MCP_LIVE_TESTS=1` opt-in at all (an accidental ungated run of `test_bulk_operations_comprehensive.py` hung and leaked to-dos and `BulkMoveTest_*`/`Cleanup Project Test - test_*` projects, cleaned up by hand). Each module now carries a module-level `pytestmark = [pytest.mark.live, pytest.mark.skipif(...)]` that skips it outright at collection time unless `THINGS_MCP_LIVE_TESTS=1` is set, plus a local `_require_live_tests_env()` helper called from every fixture that constructs an `AppleScriptManager` as a second line of defence (a plain `from conftest import _require_live_tests_env` was tried first, but `tests/integration/conftest.py` and `tests/live/conftest.py` are both bare modules named `conftest` with no `__init__.py` anywhere under `tests/`, so that import is ambiguous and broke collection - `ImportError` - whenever both directories were collected together, e.g. `pytest tests/integration tests/live`). Also fixed: the `cleanup_test_todos` fixture's project cleanup (`conftest.py`) called `update_project(id=...)` (wrong kwarg - the method takes `project_id`) and then `delete_todo(todo_id=project_id)`, whose `to do id` AppleScript target does not resolve a project id (Things' AppleScript dictionary does not treat a project as a to-do subtype for `delete`) - projects tracked via `cleanup_test_todos['project_ids']` were therefore never actually trashed, only (silently-failing-to-be) canceled. Project cleanup now cancels via the correct `project_id` kwarg and then trashes via `delete project id "..."` directly; the same trash-by-`project id` fix was applied to `test_bulk_operations_comprehensive.py`'s own `test_project` fixture. `pytest tests/integration` without `THINGS_MCP_LIVE_TESTS=1` is unchanged in effect (all skips, 122 skipped in ~0.14s) but now covers all three files, and `pytest tests/integration tests/live` (both directories together) also collects cleanly and is all skips (130 skipped); verified with `asyncio.create_subprocess_exec`/`subprocess.run`/`subprocess.Popen` patched to raise on any `osascript` argv, confirming zero real AppleScript calls are made.

...
- **`doctor` now prints exactly one "grant Full Disk Access" instruction, never two** (bead hq-gxt.13, live UX failure 2026-09-09: both "Interpreter identity" and "Claude Desktop interpreter" printed a grant instruction for different paths, and the wrong one got granted twice). `check_claude_desktop_interpreter`'s resolution is now computed once via a shared, memoized `_resolve_claude_desktop_targets()` helper consumed by both checks, so check order never matters and the (potentially slow) `uvx`/`uv` probe subprocess runs at most once per process. When Claude Desktop resolves to an interpreter that differs from the one running doctor, "Interpreter identity" is demoted to `STATUS_INFO` with no "Grant Full Disk Access" text or drag-and-drop hint at all (it explicitly says this is *not* the interpreter to grant); when they match, or nothing resolves, today's WARN/PASS + grant-line behavior is unchanged. "Claude Desktop interpreter"'s detail now leads with an unmistakable `GRANT FULL DISK ACCESS TO THIS FILE: <path>` first sentence, and its hint now notes (verified macOS 26.6) that clicking "Allow" on the "would like to access data from other apps" dialog does not persist for a Claude-Desktop-launched interpreter - only granting Full Disk Access to that exact file stops the dialog from recurring. Text output now ends with a `Full Disk Access target for Claude Desktop: <path>` footer line (one per resolved path, when any resolved); `--json` carries the same path(s) in a new top-level `full_disk_access_targets` array (empty list when none resolved).
- **`doctor`: Interpreter identity now WARNs for Homebrew framework interpreters too** (bead hq-b49, follow-up to hq-gxt.8/hq-gxt.10). `check_interpreter_identity` previously PASSed the `framework` classification even though live evidence (hq-gxt.7) showed a Homebrew-built framework interpreter's realpath embeds the formula version (e.g. `Cellar/python@3.13/3.13.15`) exactly like a `uv-managed` path, and needs the same "re-grant after upgrade" reminder. It now returns `STATUS_WARN` for `framework` as well as `uv-managed`, with detail text noting the grant must be redone after each `brew upgrade` (`venv`/`other` remain PASS).
  - **Decision** (hq-gxt.8, from hq-gxt.7 live evidence, 2026-09-09): no change to `manifest.json`, README install snippets, or `scripts/build_mcpb.sh` - the `uvx --python 3.12` launch command is unchanged. TCC keys grants on the fully resolved interpreter realpath, not on any launch form (`uvx`, `uv tool install` symlink, or a Homebrew framework build's stable `org.python.python` bundle id) - no launch form avoids re-granting Full Disk Access after an interpreter upgrade. See docs/MACOS_PERMISSIONS.md's "What we observed on a real machine" section for the cited evidence.

### Added

- **`doctor` now resolves the interpreter Claude Desktop actually launches (not just the interpreter running doctor), and probes whether the doctor process itself currently has Full Disk Access** (bead hq-gxt.11). Previously, "Interpreter identity" only reported `os.path.realpath(sys.executable)` for the process running doctor - if doctor was run from a terminal (e.g. a venv's Python) while Claude Desktop's config launches a different interpreter (e.g. `uvx` or a Homebrew framework Python), a Full Disk Access grant made to the doctor path did nothing for Claude Desktop, and the app-data TCC prompt kept recurring. Two new checks: **"Claude Desktop interpreter"** reads `~/Library/Application Support/Claude/claude_desktop_config.json` and any installed `.mcpb` extension's `manifest.json`, matches every entry whose command/args mention `things_mcp`/`mcp-server-things` (case-insensitive), and resolves the interpreter each one would actually run - a bounded (~35s total, single-probe cap 30s) `uvx`/`uv` probe subprocess for uvx-style entries (keeping only interpreter-selection flags/values and swapping the package name for a `python -c` realpath probe; a failed/timed-out probe reports INFO with the exact manual command to run), or plain path resolution (`~`/`$HOME` expansion, `shutil.which`, `os.path.realpath`) otherwise (a nonexistent resolved file is a WARN). WARNs when the resolved path differs from the interpreter running doctor, naming the Claude Desktop path (not the doctor path) to grant Full Disk Access to; INFO when the config/entry is absent. **"Full Disk Access effective (this process)"** attempts a read-only `open()` of `~/Library/Application Support/com.apple.TCC/TCC.db` - PASS if readable (own grant or inherited from the terminal), WARN on `PermissionError`, INFO if absent - explicitly noting this reflects only the doctor process, not the Claude Desktop-launched interpreter reported by the check above. Also fixes an hq-gxt.9 reviewer nit: the "Interpreter identity" drag-and-drop picker-workaround hint previously excluded the `framework` classification, even though framework realpaths also end in a patch-version segment and hit the same Launch Services picker bug - the hint now applies to every classification.

### Documentation

- **`docs/MACOS_PERMISSIONS.md`: recorded the live-verified fix and added a "Risks of granting Full Disk Access to a Python interpreter" section** (bead hq-gxt.12, evidence from hq-gxt.7). A new "Stop the dialog (verified procedure)" section states the numbered fix - grant Full Disk Access to the exact interpreter Claude Desktop launches, dragged directly from Finder - verified live (macOS 26.6, 2026-09-09/10): FDA on stops the dialog across relaunches, FDA off brings it back, and clicking Allow was tried three times across relaunches and never persisted (no sentence in the doc now claims Allow fixes recurrence). A new "Risks" section lists the four risks of a Full Disk Access grant to a Python interpreter (applies to every program run with it; the binary is ad-hoc signed so tampering can't be detected; the grant is path-keyed and silently lost on upgrades; a shared/headless machine exposes the access to anyone who can run that interpreter) with mitigations (dedicated interpreter/venv, periodic review, removal on uninstall; the `launchd` LaunchAgent route is explicitly noted as not yet verified to avoid Full Disk Access). Dialog (2)'s section now explains what it gates (Things' Group Container) and that Don't Allow only breaks reads, not writes; the "why it recurs" section now states the per-launch recurrence mechanism is not fully explained by csreq presence/absence and demotes interpreter-path instability to a secondary, better-understood cause. The interactive verification matrix and "What we observed" section now record rows (a)-(c) as observed (reboot remains untested). README's macOS-permissions summary paragraph is trimmed to 3-4 lines pointing at the verified fix and the new Risks section; `doctor`'s Full Disk Access target line now points at the Risks section too.
- **Corrected `docs/MACOS_PERMISSIONS.md`'s fix ladder: Homebrew framework Python is NOT bundle-id-stable across patch upgrades, and TCC keys on realpath, not a stable symlink** (bead hq-gxt.10). Live observation (hq-gxt.7, 2026-09-09, macOS 26.6) refuted the doc's prior recommendation to switch to a framework-build interpreter as an upgrade-proof fix: the bare interpreter binary (`.../Frameworks/Python.framework/Versions/3.13/bin/python3.13`) is a separately ad-hoc-signed stub, distinct from the stable `org.python.python` bundle id its `Python.app` carries, and produced its own new path-keyed app-data TCC grant on each of three observed Homebrew `python@3.13` patch upgrades (3.13.11, 3.13.12_1, 3.13.15) - a pre-existing `org.python.python` grant did not prevent the prompt. Fix ladder step 2 now states there is no known upgrade-proof identity for a bare python interpreter (uv-managed or framework); step 3 now states, rather than deferring to hq-gxt.5, that pinning `uv`'s launch path to a stable symlink does not help, since TCC records the fully resolved realpath the symlink points at, not the symlink path itself. Added observed matrix rows (d) (Full Disk Access drag-and-drop grant, confirmed in system TCC.db) and (e) (realpath-keying, confirmed in user TCC.db across three patch upgrades), a "What we observed on a real machine" section citing the dated TCC.db rows, and a prominent "Grant it to the RIGHT interpreter" callout pointing at `doctor`'s "Claude Desktop interpreter" check, `scripts/tcc_probe.sh`, and `readlink -f` on the configured launch command. Also reworded the `dyn.ah62d4rv4ge8xcqk` picker-bug note (hq-gxt.9 reviewer nit) to make clear the dynamic UTI is synthesized per file extension, not one fixed type, and corrected the same framework-stability claim in `doctor.py`'s "Interpreter identity" check docstring/detail text (behavior unchanged).
- **Added `docs/MACOS_PERMISSIONS.md`**, consolidating the three macOS TCC dialogs (Automation, app-data protection, Full Disk Access), why the app-data prompt can recur after a Claude Desktop restart or interpreter upgrade, and the recommended-pending-verification fix ladder for a headless/unattended Mac (bead hq-gxt.4). Replaces the "Reads fail but writes work" section previously in README.md (now a short summary linking to the new doc); linked from README's Troubleshooting section and the top of docs/TROUBLESHOOTING.md. `docs/MACOS_PERMISSIONS_FINDINGS.md` (the hq-gxt.1 investigation notes it was folded from) has been removed - its content is preserved in git history.
- **`docs/MACOS_PERMISSIONS.md` fix-ladder step 1 now documents why the Full Disk Access "+" picker greys out `python3.X` binaries and the verified drag-and-drop workaround** (bead hq-gxt.9). Observed 2026-09-09 on macOS 26.6: Launch Services parses the trailing `.11`/`.12` in the interpreter filename as a file extension rather than part of its name, so `mdls -name kMDItemContentType` reports a dynamic type (`dyn.ah62d4rv4ge8xcqk`, type tree `public.item`/`public.data` only - no `public.executable`) instead of `public.unix-executable`, and the picker's Kind column shows it as "Document", greyed out and unselectable - even though the file is a real Mach-O executable. The doc now calls this out with the `mdls` evidence and the confirmed-working fix (Cmd+Shift+G to the interpreter's directory, cancel the picker, drag the real `python3.X` file - not the `python3` alias - onto the Full Disk Access list, confirm the toggle is on). The doc now also warns to drag directly from a Finder window, never via a drag-shelf/clipboard utility (Yoink, Dropover, etc.), since those can re-stamp the file with a `com.apple.quarantine` attribute that makes Gatekeeper (`spctl`) reject it - symptom: every invocation exits instantly with `137` (SIGKILL); check with `xattr -l <path>`, fix with `xattr -d com.apple.quarantine <path>` - and includes a sub-note covering exactly this. `doctor.py`'s "Interpreter identity" check now appends the same drag-and-drop hint (plus the quarantine-risk warning) to its detail for `uv-managed`/`venv`/`other` classifications.

### Added

- **`doctor` now reports the exact interpreter binary macOS TCC keys Full Disk Access to, detects when the server was launched via Claude Desktop's `disclaimer` helper, and classifies database-open failures** (bead hq-gxt.2). Two new checks: **Interpreter identity** prints `os.path.realpath(sys.executable)` and classifies it (`uv-managed` - path embeds the interpreter's patch version, e.g. `~/.local/share/uv/python/cpython-3.12.11-.../bin/python3.12`, so the Full Disk Access grant must be redone after an interpreter upgrade; `venv`; `framework` - path lives under a `Python.framework` bundle (see hq-gxt.10: the bare interpreter binary is a separately ad-hoc-signed stub, not covered by the bundle's `org.python.python` id, so this grant must also be redone after a patch upgrade); `other`) - WARN for `uv-managed` (the path embeds the interpreter's patch version, so the grant must be redone after an upgrade); `framework` was later also made WARN (see the Changed entry above); PASS otherwise. **Launch parent** walks the process's parent chain (`ps -o ppid=,comm= -p <pid>`, up to 6 levels) and WARNs when an ancestor is `Claude.app/Contents/Helpers/disclaimer` - the TCC grant made to Claude Desktop does not extend to the launched interpreter (INFO if not found or if `ps` is unavailable, never a crash). Additionally, `check_database_readable` now does a direct `open()` of the database file before falling back to the existing `things.py` probe, classifying a `PermissionError` (errno `EPERM`/`EACCES`) as a distinct "macOS privacy (TCC) denied access" FAIL and a `FileNotFoundError` as a distinct "Things database not found" FAIL, rather than only matching the generic `sqlite3` "unable to open database file" error string. See docs/MACOS_PERMISSIONS.md for the root-cause investigation these checks are based on.
- **Read tools now surface a macOS Full Disk Access (TCC) denial reading the Things database as a distinct, actionable `database_access_denied` structured error instead of an opaque `internal_error`/generic `ToolError`** (bead hq-gxt.3). A new `tools_helpers.read_operations.is_db_access_error`/`read_error_from_exception` helper classifies a caught exception (`sqlite3.OperationalError` containing `'unable to open database file'`/`'authorization denied'`, or a plain `PermissionError`) and, when matched, returns `{"success": false, "error": "database_access_denied", "message": ..., "hint": "Grant Full Disk Access to <interpreter path>; run `mcp-server-things doctor` for diagnostics.", "interpreter": <path>}` - naming the exact interpreter binary that needs the grant. Any other exception is unaffected (still `internal_error`, or still re-raised as a `ToolError` for the read tools that already did so). See CLAUDE.md's read-tool structured error contract section for the full contract.
